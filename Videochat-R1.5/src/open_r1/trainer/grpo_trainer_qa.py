...
QUESTION_TEMPLATE_MC = """Answer the question: "[QUESTION]" according to the content of the video. Select the answer from :[OPTION].

Output your thought process within the <think> </think> tags(less than 100 words). 

Then, provide your answer within the <answer> </answer> tags, output the corresponding letter of the option. For example: <think>...</think><answer>A</answer>.
"""
...
class Qwen2VLGRPOTrainer_Video_QA(Trainer):
    ...
    def make_conversation_video(self, example):
        if 'options' in example["problem"]:
            example_prompt = QUESTION_TEMPLATE_MC.replace("[QUESTION]", example["problem"]["question"])
